...
client.on('ready', () => {
    ...
    allservers.forEach(function(i){
		...
		if(channel){channel.send('BrainBot restarted!\nВ связи с малой востребованностью бота и необходимостью освобождения хостинга для собственных проектов бот будет отключен в ближайшее время. С комментариями и предложениями обращаться к <@605817048337219597>');};
	});
